...
public class FXMLalterarCliente implements Initializable {
    ...
    @FXML
    public RadioButton sexoMascAlterarCliente;
    
    @FXML
    public RadioButton sexoFemAlterarCliente;
    ...
    @FXML
    private CheckBox segundaAlterarCliente;
    
    @FXML
    private CheckBox tercaAlterarCliente;
    
    @FXML
    private CheckBox quartaAlterarCliente;
    
    @FXML
    private CheckBox quintaAlterarCliente;
    
    @FXML
    private CheckBox sextaAlterarCliente;
    
    @FXML
    private CheckBox sabadoAlterarCliente;
    
    protected int idClienteAlterar;
    protected int idMensalidadeAlterar;
    ...
    public void alterarCliente() throws IOException {
        ...
        idMensalidadeAlterar = mensalidade.getIdMensalidade();
        ...
        if(cliente.getSexo()){
            sexoFemAlterarCliente.setSelected(false);
            sexoMascAlterarCliente.setSelected(true);
        }else{
            sexoFemAlterarCliente.setSelected(true);
            sexoMascAlterarCliente.setSelected(false);
        }
        
        if(cliente.getTreinoSegunda()){
            segundaAlterarCliente.setSelected(true);
        }else{
            segundaAlterarCliente.setSelected(false);
        }
        
        if(cliente.getTreinoTerca()){
            tercaAlterarCliente.setSelected(true);
        }else{
            tercaAlterarCliente.setSelected(false);
        }
        
        if(cliente.getTreinoQuarta()){
            quartaAlterarCliente.setSelected(true);
        }else{
            quartaAlterarCliente.setSelected(false);
        }
        
        if(cliente.getTreinoQuinta()){
            quintaAlterarCliente.setSelected(true);
        }else{
            quintaAlterarCliente.setSelected(false);
        }
        
        if(cliente.getTreinoSexta()){
            sextaAlterarCliente.setSelected(true);
        }else{
            sextaAlterarCliente.setSelected(false);
        }
        
        if(cliente.getTreinoSabado()){
            sabadoAlterarCliente.setSelected(true);
        }else{
            sabadoAlterarCliente.setSelected(false);
        }
        ...
        String valor = String.valueOf(mensalidade.getValor()).replace('.', ',');
        valorMensalidadeAlterarCliente.setText(valor + "0");
    ...
    protected Cliente coletarDadosCliente() {
        if (validarCampos()) {
            ...
            horarioTreino = horarioTreino.concat((String) minutoTreinoAlterarCliente.getValue());
            ...
            boolean sexo;
            if(sexoMascAlterarCliente.isSelected()){
                sexo = true;
            }else{
                sexo = false;
            }
            ...
            if(segundaAlterarCliente.isSelected()){
                segunda = true;
            }else{
                segunda = false;
            }
            
            if(tercaAlterarCliente.isSelected()){
                terca = true;
            }else{
                terca = false;
            }
            
            if(quartaAlterarCliente.isSelected()){
                quarta = true;
            }else{
                quarta = false;
            }
            
            if(quintaAlterarCliente.isSelected()){
                quinta = true;
            }else{
                quinta = false;
            }
            
            if(sextaAlterarCliente.isSelected()){
                sexta = true;
            }else{
                sexta = false;
            }
            
            if(sabadoAlterarCliente.isSelected()){
                sabado = true;
            }else{
                sabado = false;
            }
            ...
            cliente = new Cliente(nome, cpf, contato, observacao, endereco, bairro, cidade, estado,
                    status, email, CEP, numero, idClienteAlterar ,dataCadastro, dataNascimento, horarioTreino, sexo,
    ...
    private Mensalidade coletarDadosMensalidadeCliente(){
        if(!valorMensalidadeAlterarCliente.getText().isEmpty()){
            float valor =  (float) Double.parseDouble(valorMensalidadeAlterarCliente.getText().replace(',', '.'));
            String dia = diaVencimentoAlterarCliente.getValue().toString();
            int diaVencimento = Integer.parseInt(dia);
            Mensalidade mensalidade = new Mensalidade(valor, diaVencimento, idMensalidadeAlterar);
            return mensalidade;
        }
        return null;
    }
